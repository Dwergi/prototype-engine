...
namespace dd
{
	...
	ddc::Entity TestEntities::CreateBall(ddc::World& world, glm::vec3 translation, glm::vec4 colour, float size)
	{
		...
		dd::BoundSphereComponent& bound_sphere = world.Add<dd::BoundSphereComponent>(entity);
		bound_sphere.Sphere.Radius = 1.0f;

		dd::PhysicsSphereComponent& physics_sphere = world.Add<dd::PhysicsSphereComponent>(entity);
	...
	void TestEntities::CreatePhysicsPlaneTestScene(ddc::World& world)
	{
		float sphere_size = 3;
		...
		balls.Add(CreateBall(world, ball_positions[0], glm::vec4(0.2, 0.2, 0.8, 1), sphere_size));
		balls.Add(CreateBall(world, ball_positions[1], glm::vec4(0.8, 0.2, 0.8, 1), sphere_size));
		balls.Add(CreateBall(world, ball_positions[2], glm::vec4(0.2, 0.8, 0.2, 1), sphere_size));
		balls.Add(CreateBall(world, ball_positions[3], glm::vec4(0.2, 0.8, 0.8, 1), sphere_size));
...

		{
			...
			phys_plane_proto->PopulateFromEntity(plane, world);
		}

		{
			ddc::Entity plane = phys_plane_proto->Instantiate(world);
			dd::ColourComponent* clr = world.Access<dd::ColourComponent>(plane);
			clr->Colour = glm::vec4(0.8, 0.2, 0.2, 1);

			dd::TransformComponent* transform_cmp = world.Access<dd::TransformComponent>(plane);
		...
		dd::Service<dd::InputBindings>()->RegisterHandler(dd::InputAction::RESET_PHYSICS, 
			[balls, ball_positions, &world](dd::InputAction action, dd::InputType type)
			{
	...
	void TestEntities::CreateAxes(ddc::World& world)
	{
		{
			ddc::Entity x_entity = world.CreateEntity<dd::RayComponent, dd::ColourComponent>();
			world.AddTag(x_entity, ddc::Tag::Visible);

			dd::RayComponent* x_ray = world.Access<dd::RayComponent>(x_entity);
			x_ray->Ray = ddm::Ray(glm::vec3(-50, 0, 0), glm::vec3(1, 0, 0), 100);

			dd::ColourComponent* x_colour = world.Access<dd::ColourComponent>(x_entity);
			x_colour->Colour = glm::vec4(1, 0, 0, 1);
		}

		{
			ddc::Entity y_entity = world.CreateEntity<dd::RayComponent, dd::ColourComponent>();
			world.AddTag(y_entity, ddc::Tag::Visible);

			dd::RayComponent* y_ray = world.Access<dd::RayComponent>(y_entity);
			y_ray->Ray = ddm::Ray(glm::vec3(0, -50, 0), glm::vec3(0, 1, 0), 100);

			dd::ColourComponent* y_colour = world.Access<dd::ColourComponent>(y_entity);
			y_colour->Colour = glm::vec4(0, 1, 0, 1);
		}

		{
			ddc::Entity z_entity = world.CreateEntity<dd::RayComponent, dd::ColourComponent>();
			world.AddTag(z_entity, ddc::Tag::Visible);

			dd::RayComponent* z_ray = world.Access<dd::RayComponent>(z_entity);
			z_ray->Ray = ddm::Ray(glm::vec3(0, 0, -50), glm::vec3(0, 0, 1), 100);

			dd::ColourComponent* z_colour = world.Access<dd::ColourComponent>(z_entity);
