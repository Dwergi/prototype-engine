...
namespace dd
{
	...
	void AABBOctree::Remove(OctreeEntry entry)
	{
		...
		{
			uint64 index = node.m_data.Find(entry);
			if (index != dd::InvalidIndex)
			{
	...
	//
	uint64 AABBOctree::GetAllContaining(const glm::vec3& pt, Vector<OctreeEntry>& outResults) const
	{
		outResults.Clear();
	...
	//
	uint64 AABBOctree::GetAllIntersecting(const ddm::AABB& search_bounds, Vector<OctreeEntry>& outResults) const
	{
		outResults.Clear();
