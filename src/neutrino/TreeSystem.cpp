...
namespace neut
{
		...
		ddc::System( "Trees" )
	{
		...
		RequireWrite<neut::TreeComponent>();
		RequireWrite<dd::BoundBoxComponent>();
		RequireWrite<dd::LinesComponent>();
	}

	static void InitializeTree( neut::TreeComponent& tree )
	{
	...
	void TreeSystem::Initialize( ddc::EntityLayer& entities )
	{
		...
		neut::TreeComponent& tree = entities.Add<neut::TreeComponent>( entity );
		InitializeTree( tree );
	}

	static void BranchParticle( neut::TreeComponent& tree, neut::TreeParticle& particle )
	{
		if( tree.Particles.size() > 1024 )
			return;

		neut::TreeParticle& branched = tree.Particles.emplace_back();
	...
	static void UpdateTree( neut::TreeComponent& tree, dd::BoundBoxComponent& bound_box, dd::LinesComponent& lines, const dd::TransformComponent& transform, const float delta_t )
	{
