...
namespace dd
{
	dd::Service<dd::JobSystem> s_jobsystem;
	...
	{
		ddc::EntityType physics_sphere = ddc::EntityType::Make<dd::TransformComponent, dd::PhysicsSphereComponent>();
		entities.CreateEntityType(physics_sphere);
	}

	static void AddSphereToBVH(BVHTree& bvh, const ddm::Sphere& sphere, const glm::vec3& displacement, const glm::mat4& transform)
	{
		ddm::Sphere transformed = sphere.GetTransformed(transform);
		ddm::AABB bounds(transformed);
		bounds.Expand(bounds.Min + displacement);
		bounds.Expand(bounds.Max + displacement);
		bvh.Add(bounds);
	}
	...
	thread_local std::vector<BVHHandle> s_hits;
	...
	void PhysicsSystem::PartialUpdate(const ddc::UpdateData* update_data, uint start, uint end)
	{
		DD_PROFILE_SCOPED(PhysicsSystem_PartialUpdate);

		const float delta_t = update_data->Delta();

		const auto& static_planes = update_data->Data("static_planes");
		const auto& static_spheres = update_data->Data("static_spheres");
		const auto& dynamic_spheres = update_data->Data("dynamic_spheres");
		...
		for (uint sphere_idx = start; sphere_idx < end; ++sphere_idx)
		{
			...
			// hit tests
			DD_TODO("This could be much easier by adding everything to the broadphase and then resolving the shapes later.");
	...
	void PhysicsSystem::Update(const ddc::UpdateData& update)
	{
		...
		{
			uint start = job_size * i;
			uint end = start + job_size;
			if (i == s_jobsystem->Threads() - 1)
			{
				end = dynamic_spheres.Size();
			}

			dd::Job* job = s_jobsystem->CreateMethodChild(root, this, &PhysicsSystem::PartialUpdate, &update, start, end);
			s_jobsystem->Schedule(job);
		}

		s_jobsystem->Schedule(root);
		s_jobsystem->Wait(root);
	}
