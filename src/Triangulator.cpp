...
namespace dd
{
	Triangulator::Triangulator(const ddr::Mesh& mesh)
	{
	...
	Triangulator::Triangulator(const dd::ConstBuffer<glm::vec3>& pos)
	{
		m_positions = pos.Get();
		m_size = pos.Size() / 3;
	}

	Triangulator::Triangulator(const dd::ConstBuffer<glm::vec3>& pos, const dd::ConstBuffer<uint>& idx)
	{
		if (idx.IsValid())
		{
			m_positions = pos.Get();
	...
	Triangulator::Triangulator(const std::vector<glm::vec3>& pos)
	{
		m_positions = pos.data();
		m_size = pos.size() / 3;
	}

	Triangulator::Triangulator(const std::vector<glm::vec3>& pos, const std::vector<uint>& idx)
	{
		if (!idx.empty())
		{
			m_positions = pos.data();
	...
	ConstTriangle Triangulator::operator[](size_t i) const
	{
		DD_ASSERT(i < Size());
