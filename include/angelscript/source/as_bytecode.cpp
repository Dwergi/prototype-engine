...
void asCByteCode::ClearAll()
{
	...
	{
		first = del->next;
		m_engine->memoryMgr.FreeByteInstruction(del);
		del = first;
	}
...
void asCByteCode::OptimizeLocally(const asCArray<int> &tempVariableOffsets)
{
	...
	if( !m_engine->ep.optimizeByteCode )
		return;
	...
	if( last && last->op == asBC_LOADOBJ && IsTemporary(last->wArg[0]) )
	{
		...
		asCByteInstruction *instruction = last->prev;
		asASSERT( instruction && instruction->op == asBC_Block );
		instruction = instruction->prev;
		while( instruction && instruction->op == asBC_FREE )
		{
			freedVars.PushLast(instruction->wArg[0]);
			instruction = instruction->prev;
		}
		...
		if( instruction && instruction->op == asBC_Block )
		{
			// We expect a sequence PshVPtr, RefCpyV, PopPtr just before the clean up block
			instruction = instruction->prev;
			if( instruction && instruction->op == asBC_PopPtr ) instruction = instruction->prev;
			if( instruction && instruction->op == asBC_RefCpyV && instruction->wArg[0] == tempVar ) instruction = instruction->prev;
			if( instruction && instruction->op == asBC_PshVPtr && freedVars.Exists(instruction->wArg[0]) )
			{
				// Update the LOADOBJ to load the local variable directly
				tempVar = instruction->wArg[0];
				last->wArg[0] = tempVar;

				// Remove the copy of the local variable into the temp
				DeleteInstruction(instruction->next); // deletes RefCpyV
				DeleteInstruction(instruction->next); // deletes PopPtr
				DeleteInstruction(instruction);       // deletes PshVPtr

				// Find and remove the FREE instruction for the local variable too
				instruction = last->prev->prev;
				while( instruction )
				{
					asASSERT( instruction->op == asBC_FREE );
					if( instruction->wArg[0] == tempVar )
					{
...
void asCByteCode::Optimize()
{
	...
	while( instr )
	{
		...
		if( currOp == asBC_JitEntry && !m_engine->ep.includeJitInstructions )
		{
...
void asCByteCode::ExtractLineNumbers()
{
	...
	while( instr )
	{
		...
		if( curr->op == asBC_LINE )
		{
			...
			if( !m_engine->ep.buildWithoutLineCues )
			{
...
{
	void *ptr = m_engine->memoryMgr.AllocByteInstruction();
	if( ptr == 0 )
	{
		// Out of memory
		return 0;
...
asCByteInstruction *asCByteCode::DeleteInstruction(asCByteInstruction *instr)
{
	...
	m_engine->memoryMgr.FreeByteInstruction(instr);
...
void asCByteCode::DebugOutput(const char *name, asCScriptEngine *engine, asCScriptFunction *func)
{
	...
	if( func->objectType )
	{
		fprintf(file, " %.3d: %s this\n", 0, func->objectType->m_name.AddressOf());
		offset -= AS_PTR_SIZE;
	}
	...
	for( n = 0; n < func->scriptData->objVariablePos.GetLength(); n++ )
	{
		...
		if( !found )
		{
			if( func->scriptData->objVariableTypes[n] )
			{
				...
				bool isOnHeap = asUINT(idx) < func->scriptData->objVariablesOnHeap ? true : false;
				fprintf(file, " %.3d: %s%s {noname}\n", func->scriptData->objVariablePos[n], isOnHeap ? "(heap) " : "", func->scriptData->objVariableTypes[n]->m_name.AddressOf());
			}
			else
	...
	while( instr )
	{
		...
		case asBCTYPE_W_ARG:
			if( instr->op == asBC_STR )
			{
				...
				const asCString &constantStr = engine->GetConstantString(id);
				fprintf(file, "   %-8s %d         (l:%ld s:\"%.10s\")\n", asBCInfo[instr->op].name, asWORD(instr->wArg[0]), (long int)constantStr.GetLength(), constantStr.AddressOf());
			}
			else
		...
		case asBCTYPE_DW_ARG:
			...
			case asBC_FuncPtr:
				{
					asCScriptFunction *function = *(asCScriptFunction**)ARG_DW(instr->arg);
					fprintf(file, "   %-8s 0x%x          (func:%s)\n", asBCInfo[instr->op].name, (asUINT)*ARG_DW(instr->arg), function->GetDeclaration());
				}
				break;
		...
		case asBCTYPE_QW_ARG:
			...
			case asBC_FuncPtr:
				{
					asCScriptFunction *scriptFunc = *(asCScriptFunction**)ARG_QW(instr->arg);
					fprintf(file, "   %-8s 0x%x          (func:%s)\n", asBCInfo[instr->op].name, (asUINT)*ARG_QW(instr->arg), scriptFunc->GetDeclaration());
				}
				break;
		...
		case asBCTYPE_DW_DW_ARG:
			...
			{
				asCObjectType *ot = *(asCObjectType**)ARG_DW(instr->arg);
				asCScriptFunction *scriptFunc = engine->scriptFunctions[instr->wArg[0]];
				fprintf(file, "   %-8s 0x%x, %d             (type:%s, %s)\n", asBCInfo[instr->op].name, *(int*)ARG_DW(instr->arg), *(int*)(ARG_DW(instr->arg)+1), ot->GetName(), scriptFunc ? scriptFunc->GetDeclaration() : "{no func}");
			}
			else
...
#ifdef __GNUC__
#ifdef AS_64BIT_PTR
				fprintf(file, "   %-8s 0x%lx, %d             (type:%s, %s)\n", asBCInfo[instr->op].name, *(asINT64*)ARG_QW(instr->arg), *(int*)(ARG_DW(instr->arg)+2), ot->GetName(), scriptFunc ? scriptFunc->GetDeclaration() : "{no func}");
#else
...
int asCByteCode::RemoveLastInstr()
{
	...
	if( first == last )
	{
		m_engine->memoryMgr.FreeByteInstruction(last);
		first = 0;
		last = 0;
	}
	else
	{
		...
		bc->Remove();
		m_engine->memoryMgr.FreeByteInstruction(bc);
	}
