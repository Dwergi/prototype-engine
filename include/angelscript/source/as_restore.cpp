...
int asCReader::ReadInner() 
{
	...
	{
		bool isNew;
		asCScriptFunction *scriptFunc = ReadFunction(isNew, false, true);
		if( scriptFunc )
		{
			scriptFunc->module = module;
			module->funcDefs.PushLast( scriptFunc );
			engine->funcDefs.PushLast( scriptFunc );
			...
			if( scriptFunc->isShared )
			{
				for( asUINT n = 0; n < engine->funcDefs.GetLength(); n++ )
				{
					asCScriptFunction *f2 = engine->funcDefs[n];
					if( f2 == 0 || scriptFunc == f2 )
						continue;

					if( !f2->isShared )
						continue;

					if( f2->m_name == scriptFunc->m_name &&
						f2->nameSpace == scriptFunc->nameSpace &&
						f2->IsSignatureExceptNameEqual( scriptFunc ) )
					{
						// Replace our funcdef for the existing one
						module->funcDefs[module->funcDefs.IndexOf( scriptFunc )] = f2;
						f2->AddRefInternal();

						engine->funcDefs.RemoveValue( scriptFunc );

						savedFunctions[savedFunctions.IndexOf( scriptFunc )] = f2;
	...
	if( !error )
	{
		for( i = 0; i < usedTypes.GetLength() && !error; i++ )
		{
			...
			if( !engine->CallGlobalFunctionRetBool(usedTypes[i], &dontGarbageCollect, callback->sysFuncIntf, callback) )
			{
				...
				asCString str;
				str.Format(TXT_INSTANCING_INVLD_TMPL_TYPE_s_s, usedTypes[i]->m_name.AddressOf(), sub.AddressOf());
				engine->WriteMessage("", 0, 0, asMSGTYPE_ERROR, str.AddressOf());
				Error(TXT_INVALID_BYTECODE_d);
			}
			else
		...
		else
		{
			...
			if( c == 'm' )
			{
				...
				else if( func.funcType == asFUNC_FUNCDEF )
				{
					...
					for( asUINT i = 0; i < funcs.GetLength(); i++ )
					{
						...
						if( f == 0 || func.m_name != f->m_name || !func.IsSignatureExceptNameAndObjectTypeEqual(f) )
							continue;
			...
			else
			{
				...
				else if( func.m_name[0] == '$' )
				{
					...
					else if( func.m_name == "$fact" || func.m_name == "$beh3" )
					{
				...
				else if( func.objectType == 0 )
				{
					...
					const asCArray<asUINT> &funcs = engine->registeredGlobalFuncs.GetIndexes(func.nameSpace, func.m_name);
					for( asUINT i = 0; i < funcs.GetLength(); i++ )
					{
...
void asCReader::ReadFunctionSignature(asCScriptFunction *func)
{
	...
	ReadString(&func->m_name);
	if( func->m_name == DELEGATE_FACTORY )
	{
...
asCScriptFunction *asCReader::ReadFunction(bool &isNew, bool addToModule, bool addToEngine, bool addToGC) 
{
	...
	if( func->funcType == asFUNC_SCRIPT )
	{
		...
		if( !noDebugInfo )
		{
			asUINT paramCount = asUINT(ReadEncodedUInt64());
			if( paramCount > func->parameterTypes.GetLength() )
			{
				error = true;
				func->DestroyHalfCreated();
				return 0;
	...
	else if( phase == 2 )
	{
		...
		else
		{
			...
			if( !ot->IsInterface() && ot->flags != asOBJ_TYPEDEF && ot->flags != asOBJ_ENUM )
			{
				...
				{
					bool isFuncNew;
					asCScriptFunction *scriptFunc = ReadFunction(isFuncNew, !sharedExists, !sharedExists, !sharedExists);
					if( scriptFunc )
					{
						if( sharedExists )
						{
							// Find the real function in the object, and update the savedFunctions array
							bool found = false;
							for( asUINT c = 0; c < ot->beh.constructors.GetLength(); c++ )
							{
								asCScriptFunction *realFunc = engine->GetScriptFunction(ot->beh.constructors[c]);
								if( realFunc->IsSignatureEqual(scriptFunc) )
								{
									// If the function is not the last, then the substitution has already occurred before
									if( savedFunctions[savedFunctions.GetLength()-1] == scriptFunc )
										savedFunctions[savedFunctions.GetLength()-1] = realFunc;
									found = true;
									module->scriptFunctions.PushLast(realFunc);
									realFunc->AddRefInternal();
							...
							if( isFuncNew )
							{
								// Destroy the function without releasing any references
								scriptFunc->id = 0;
								scriptFunc->scriptData->byteCode.SetLength(0);
								scriptFunc->ReleaseInternal();
							}
						}
						else
						{
							ot->beh.constructors.PushLast(scriptFunc->id);
							scriptFunc->AddRefInternal();

							if( scriptFunc->parameterTypes.GetLength() == 0 )
								ot->beh.construct = scriptFunc->id;
						}
					}
					else
					{
					...
					scriptFunc = ReadFunction(isFuncNew, !sharedExists, !sharedExists, !sharedExists);
					if( scriptFunc )
					{
						if( sharedExists )
						{
							// Find the real function in the object, and update the savedFunctions array
							bool found = false;
							for( asUINT r = 0; r < ot->beh.factories.GetLength(); r++ )
							{
								asCScriptFunction *realFunc = engine->GetScriptFunction(ot->beh.factories[r]);
								if( realFunc->IsSignatureEqual(scriptFunc) )
								{
									// If the function is not the last, then the substitution has already occurred before
									if( savedFunctions[savedFunctions.GetLength()-1] == scriptFunc )
						...
						{
							ot->beh.factories.PushLast(scriptFunc->id);
							scriptFunc->AddRefInternal();

							if( scriptFunc->parameterTypes.GetLength() == 0 )
								ot->beh.factory = scriptFunc->id;
						}
					}
					else
					{
			...
			for( n = 0; n < size; n++ ) 
			{
				...
				if( func )
				{
					if( sharedExists )
					{
						...
						{
							asCScriptFunction *realFunc = engine->GetScriptFunction(ot->methods[m]);
					...
					else
					{
						...
						if( func->m_name == "opAssign" && func->parameterTypes.GetLength() == 1 &&
			...
			for( n = 0; n < size; n++ )
			{
				...
				if( func )
				{
					if( sharedExists )
					{
						// Find the real function in the object, and update the savedFunctions array
						bool found = false;
						for( asUINT vf = 0; vf < ot->virtualFunctionTable.GetLength(); vf++ )
						{
							asCScriptFunction *realFunc = ot->virtualFunctionTable[vf];
							if( realFunc->IsSignatureEqual(func) )
							{
								// If the function is not the last, then the substitution has already occurred before
								if( savedFunctions[savedFunctions.GetLength()-1] == func )
...
void asCReader::ReadDataType(asCDataType *dt) 
{
	...
	if( tokenType == ttIdentifier && objType && objType->m_name == "$func" )
	{
		asCScriptFunction func(engine, module, asFUNC_DUMMY);
		ReadFunctionSignature(&func);
		if( error ) return;
		for( asUINT rf = 0; rf < engine->registeredFuncDefs.GetLength(); rf++ )
		{
			// TODO: access: Only return the definitions that the module has access to
			if( engine->registeredFuncDefs[rf]->m_name == func.m_name &&
				engine->registeredFuncDefs[rf]->nameSpace == func.nameSpace )
			{
				funcDef = engine->registeredFuncDefs[rf];
				break;
			}
		}
		...
		if( !funcDef && module )
		{
			for( asUINT fd = 0; fd < module->funcDefs.GetLength(); fd++ )
			{
				if( module->funcDefs[fd]->m_name == func.m_name &&
					module->funcDefs[fd]->nameSpace == func.nameSpace )
				{
					funcDef = module->funcDefs[fd];
					break;
				}
			}
		}
...
asCObjectType* asCReader::ReadObjectType() 
{
	...
	else if( ch == 's' )
	{
		...
		{
			if( engine->templateSubTypes[n] && engine->templateSubTypes[n]->m_name == typeName )
			{
	...
	else if( patternNode->type == asLPT_TYPE )
	{
		...
		if( dt.GetTokenType() == ttQuestion )
		{
			if( nextTypeId != -1 )
			{
				...
				asCDataType dataType = patternType->engine->GetDataTypeFromTypeId(nextTypeId);
				asUINT size;
				if( dataType.IsObjectHandle() || (dataType.GetObjectType() && (dataType.GetObjectType()->flags & asOBJ_REF)) )
					size = AS_PTR_SIZE*4;
				else
					size = dataType.GetSizeInMemoryBytes();
		...
		else
		{
			...
			asUINT size;
			asCDataType dataType = reinterpret_cast<asSListPatternDataTypeNode*>(patternNode)->dataType;
			if( dataType.IsObjectHandle() || (dataType.GetObjectType() && (dataType.GetObjectType()->flags & asOBJ_REF)) )
				size = AS_PTR_SIZE*4;
			else
				size = dataType.GetSizeInMemoryBytes();
...
void asCWriter::WriteFunction(asCScriptFunction* func) 
{
	...
	if( func->funcType == asFUNC_SCRIPT )
	{
		...
		if( !stripDebugInfo )
		{
			asUINT paramCount = asUINT(func->parameterNames.GetLength());
			WriteEncodedInt64( paramCount );
			for( asUINT n = 0; n < paramCount; n++ )
...
void asCWriter::WriteDataType(const asCDataType *dt) 
{
	...
	if( t == ttIdentifier && dt->GetObjectType()->m_name == "$func" )
	{
