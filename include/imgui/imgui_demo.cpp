...
void ImGui::ShowTestWindow(bool* opened)
{
    ...
    if (ImGui::CollapsingHeader("Window options"))
    {
        ...
        if (ImGui::TreeNode("Fonts", "Fonts (%d)", ImGui::GetIO().Fonts->Fonts.Size))
        {
            ...
            for (__int64 i = 0; i < atlas->Fonts.Size; i++)
            {
                ImFont* font = atlas->Fonts[ (int) i];
                ...
                if (i > 0) { ImGui::SameLine(); if (ImGui::SmallButton("Set as default")) { atlas->Fonts[ (int) i] = atlas->Fonts[0]; atlas->Fonts[0] = font; } }
                ImGui::PushFont(font);
